...
function getẋ(model::Arthur, x::AbstractVector{T}) where T
    ...
    ẋ = 1000 * J.linear*x[SA[8, 9, 10, 11, 12, 13, 14]]
    return ẋ'ẋ
end

function getẋ_jacobian(model::Arthur, x::AbstractVector)
    return ForwardDiff.gradient(dx -> getẋ(model, dx), x)
end
...
struct VelNormConstraint{S,D,T} <: TrajectoryOptimization.StageConstraint
	n::Int
	m::Int
	val::T
	sense::S
	inds::SVector{D,Int}
    params::MPC_Params
	function VelNormConstraint(n::Int, m::Int, val::T, sense::TrajectoryOptimization.ConstraintSense,
			inds=SVector{n+m}(1:n+m); params=MPC_Params()) where T
		if inds == :state
			inds = SVector{n}(1:n)
		elseif inds == :control
		...
		new{typeof(sense),length(inds),T}(n,m,val,sense,inds,params)
	end
end

@inline state_dim(con::VelNormConstraint) = con.n
@inline control_dim(con::VelNormConstraint) = con.m
@inline sense(con::VelNormConstraint) = con.sense
@inline Base.length(::VelNormConstraint) = 1

function evaluate(con::VelNormConstraint, z::AbstractKnotPoint)
	x = z.z[con.inds]
    # params = con.params
    # set_configuration!(params.state, x[1:7])
    ...
    # ẋ = 1000 * J.linear*x[8:14]
    
	return @SVector [getẋ(con.params.model, x) - con.val*con.val]
end

function jacobian!(∇c, con::VelNormConstraint, z::AbstractKnotPoint)
	x = z.z[con.inds]
	∇c[1,con.inds] .= getẋ_jacobian(con.params.model, x)
	return false
end

function change_dimension(con::VelNormConstraint, n::Int, m::Int, ix=1:n, iu=1:m; params=MPC_Params())
	VelNormConstraint(n, m, con.val, con.sense, ix[con.inds], params=params)
end
...
function ArthurHorizonProblem(Xref, x0, N; start=1, params=MPC_Params())
    ...
    # Cartesian Velocity Bound
    ẋ_max = 5 # mm/s
    vel_bnd = VelNormConstraint(n, m, ẋ_max, Inequality(), 1:14, params=params)
    add_constraint!(cons, vel_bnd, 1:N-1)
